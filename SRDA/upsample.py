...
def upsample_restarts(source_path, destination_path, date, start_mem, end_mem):
    """
    ...
    for mem in range(start_mem, end_mem + 1):
        ...
        interpolation.upsample_restart_V2(file, destination_file)

def upsample_icefiles(source_path, destination_path, date, start_mem, end_mem):
    """
    Upsamples restart files from a source directory and saves them to a destination directory.
    ...
    for mem in range(start_mem, end_mem + 1):
        ...
        if not(os.path.exists(destination_file) and os.path.getsize(destination_file) == expected_size):
        # If the file doesn't exist or doesn't have the right size, then upsample 
            interpolation.upsample_ncfile(file, destination_file)
...
if __name__ == "__main__":
    ...
    source_path = '/cluster/work/users/antber/TP2a0.10/expt_02.5/data/'
    #source_path = '/cluster/work/users/antber/TP2_Reanalysis/TOBACKUP/' + julday + '/FORECAST/'
    destination_path = '/cluster/work/users/antber/TP2a0.10/expt_02.5/LR_upsampled/'
    #source_path = '/cluster/work/users/antber/TP2a0.10/expt_02.4/LR/'
    #destination_path = '/cluster/work/users/antber/TP2a0.10/expt_02.4/LR_upsampled/'
    upsample_restarts(source_path, destination_path, args.date, args.start_mem, args.end_mem)

    source_path_ice = '/cluster/work/users/antber/TP2a0.10/expt_02.5/data/cice/'
    #source_path_ice = '/cluster/work/users/antber/TP2_Reanalysis/TOBACKUP/' + julday + '/FORECAST/'
    destination_path_ice = '/cluster/work/users/antber/TP2a0.10/expt_02.5/LR_upsampled/cice/'
    upsample_icefiles(source_path_ice, destination_path_ice, args.date, args.start_mem, args.end_mem)
